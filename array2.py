...
class Solution:
    def hIndex(self, citations: List[int]) -> int:
        citations.sort(reverse=True)  # Sort citations array in non-decreasing order
        h_index = 0
        for i, citation_count in enumerate(citations):
            if citation_count >= i + 1:  # Check if the citation count is greater than or equal to its index + 1
                h_index = i + 1
            else:
                break
        return h_index
...
class Solution:
    def productExceptSelf(self, nums: List[int]) -> List[int]:
        ...
        # Calculate products to the left and store them in the result array
        left_product = 1
        for i in range(length):
